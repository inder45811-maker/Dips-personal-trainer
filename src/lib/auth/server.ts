...
const PROD_HOSTS = ["hskcoaching.co.uk", "www.hskcoaching.co.uk"];
const PROD_ORIGINS = [
  "https://hskcoaching.co.uk",
  "https://www.hskcoaching.co.uk",
];
const baseURL = explicitBaseURL ?? {
  // Include loopback hosts so dynamic baseURL resolves for local email/password
  // (not only the preview wildcard).
  allowedHosts: [...previewAllowedHosts, "localhost", "127.0.0.1", "[::1]", ...PROD_HOSTS],
...
const trustedOrigins: string[] = explicitBaseURL
  ? [explicitBaseURL, ...LOCAL_DEV_ORIGINS, ...PROD_ORIGINS]
  : [
